...
class PrefillPPPipeline(PPPipeline, ABC):
    ...
    """
    @abstractmethod
    def X_prefill_rank(self, r: int) -> float:
    ...
    # TODO: Update this math to model the fact that T_compute overlaps with (X_handoff + X_prefill_activations).
    def T_prefill_completion(self) -> float:
        """
        Calculates prefill completion latency, which measures the time until the prefill cluster 
        finishes its work.  Use the Pipeline Execution Model:
        t_pipeline_completion = (PP + K - 1) * max(T_stage) + T_handoff_last_rank
        """
        # 1. Identify the bottleneck stage (Clock Cycle of the pipeline)
        stage_latencies = [self.X_prefill_rank(r) for r in range(self.PP)]
        t_max_stage = max(stage_latencies)

        # 2. Pipeline bubble formula
        # It takes (PP - 1) stages to reach the last rank, 
        # plus (K) micro-batches to finish the work at that rank.
        t_pipeline_completion = (self.PP + self.K - 1) * t_max_stage

        # 3. Add the final handoff from the last prefill rank to the decode cluster
        # In PD disaggregation, the first token can't be generated until 
        # the final prefill activations reach the decoder.
        return t_pipeline_completion + self.X_handoff(self.PP - 1)
...
class DecodePPPipeline(PPPipeline, ABC):
    ...
    def TPOT(self) -> float:
        ...
        for r in range(self.PP - 1):
            stage_latencies.append(self.X_decode(r))
...
class AkamaiDeployment(PrefillPPPipeline, DecodePPPipeline):
    ...
    # TODO: Decouple T_prefill from this method.
    # TODO: Rename this method to something like X_prefill_activations.
    def X_prefill_rank(self, r: int) -> float:
        """
    ...
    def TTFT(self) -> float:
        ...
        if self.decode_rank_model is None:
            raise AttributeError("decode_rank_model must be set before calling T_decode().")
        t_first_token_path = sum(self.X_decode(r) for r in range(self.PP - 1)) + self.decode_rank_model.T_decode()
